...
const Fertilizer = ({ setCartItems }) => {
  ...
  useEffect(() => {
    const fetchVegetables = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/products/type/fertilizer');
    ...
    <div className="home-container">
      <header className="header">
        <nav className="nav">
          <ul>
            ...
            <li><Link to="/fertilizers">Drinks</Link></li>
      ...
      <div className="product-grid">
        {filteredProducts.length > 0 ? (
          ...
          <p>No Drinks products available at the moment.</p>
        )}
      </div>
    </div>
  );
