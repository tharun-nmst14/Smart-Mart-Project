...
const Hardware = ({ setCartItems }) => {
  ...
  useEffect(() => {
    const fetchVegetables = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/products/type/hardware');
    ...
    <div className="home-container">
      ...
      <div className="product-grid">
        {filteredProducts.length > 0 ? (
          ...
          <p>No Hardware products available at the moment.</p>
        )}
      </div>
    </div>
  );
